...
def extract_pages_filePaths(html_file_path, remove_original_after=True):
    pdf_file_name = html_file_path.split("/")[-1].split(".")[0]
    ...
    for index, svg in enumerate(svgs):
        ...
        new_soup = BeautifulSoup(
            '<!DOCTYPE html><html><head><meta charset="utf-8"><title>SVG Page</title></head><body style="margin:0',
            "lxml",
        )
        ...
        output_file_path = os.path.join(f"{pdf_file_name}_page_{index + 1}.html")

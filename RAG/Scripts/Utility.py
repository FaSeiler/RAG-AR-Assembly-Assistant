...
def ExtractPageNumbers(text):
    ...
    for match in matches:
        if match[0]:  # single page number, matched by "page <int>"
            ...
            9
        ]:  # multiple page numbers, matched by "Page Numbers: <int>, <int>, ..."
            numbers = re.findall(
                r"\d+", match[9]
            ...
            match[13] and match[14]
        ):  # page range, matched by "page_number: <int>-<int>"
            start, end = int(match[13]), int(match[14])
            page_numbers.update(range(start, end + 1))
        elif (
            match[15] and match[16]
        ):  # page range, matched by "Page Number: <int>-<int>"
            start, end = int(match[15]), int(match[16])
            page_numbers.update(range(start, end + 1))
        elif (
            match[17] and match[18]
        ):  # page range, matched by "page_number: <int>-<int>"
